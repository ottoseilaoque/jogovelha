  ...
  function atualizarStatus() {
      ...
      if (vencedor) {
          ...
          statusJogo.style.color = '#ff0080'; // Cor para o vencedor
  ...
  function reiniciarJogo() {
      ...
      celulas.forEach(celula => {
          celula.textContent = ''; // Limpa o texto dos quadrados
          celula.classList.remove('tomada'); // remove a classe 'tomada'
      });
  ...
  celulas.forEach(celula => {
      celula.addEventListener('click', aoClicar); // Quando um quadrado for clicado, chama a função 'aoClicar'
  });
